...
void QStreamDecoder::initialize()
{
	...
	// Allocate decoder
	mCodec = avcodec_find_decoder(mIsAudio ? CODEC_ID_AAC :
		CODEC_ID_H264);
	if (!mCodec)
	{
	...
	// open codec
	if (avcodec_open2(mCodecCtx, mCodec, NULL))
	{
	...
	if (mIsAudio)
	{
		...
		mResampleCtx = swr_alloc();
		av_opt_set_int(mResampleCtx, "in_channel_layout", AV_CH_LAYOUT_STEREO, 0);
		av_opt_set_int(mResampleCtx, "out_channel_layout", AV_CH_LAYOUT_STEREO, 0);
		av_opt_set_int(mResampleCtx, "in_sample_rate", 48000, 0);
		av_opt_set_int(mResampleCtx, "out_sample_rate", 48000, 0);
		av_opt_set_sample_fmt(mResampleCtx, "in_sample_fmt", AV_SAMPLE_FMT_FLTP, 0);
		av_opt_set_sample_fmt(mResampleCtx, "out_sample_fmt", AV_SAMPLE_FMT_S16, 0);
...
bool QStreamDecoder::decodeVideoFrame(unsigned char* bytes, int size)
{
	...
	while (mPacket.size > 0)
	{
		len = avcodec_decode_video2(mCodecCtx, mPicture, &got_picture, &mPacket);
		...
		if (got_picture)
		{
			if (!mPictureRGB || mPictureRGB->width != mCodecCtx->width || mPictureRGB->height != mCodecCtx->height)
			{
				...
				int numBytes = avpicture_get_size(PIX_FMT_RGB24, mCodecCtx->width, mCodecCtx->height);
				...
				avpicture_fill((AVPicture *)mPictureRGB, mRGBBuffer, PIX_FMT_RGB24,
			...
			if (mLastRendered)
			{
				mConvertCtx = sws_getCachedContext(mConvertCtx, w, h, mCodecCtx->pix_fmt, w, h, PIX_FMT_RGB24, SWS_BICUBIC, NULL, NULL, NULL);
